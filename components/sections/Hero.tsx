...
const DEFAULTS = {
  overline: '[ 3D VISUALIZATION · MOTION DESIGN · ENGINEERING ]',
  ...
  stats: ['50+ Projects', '5 Years Experience', 'Industrial & Medical Specialist'],
};
...
export default function Hero({ overline, tagline, subtext, stats }: HeroProps) {
  ...
  const displayOverline = overline || DEFAULTS.overline;
  ...
  return (
    <section ref={heroRef} className="relative min-h-[100dvh] flex items-center justify-center overflow-hidden mesh-bg w-full max-w-[100vw]">
      ...
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full bg-[var(--accent-secondary)] opacity-[0.04] blur-[120px] animate-float pointer-events-none" style={{ animationDelay: '2s' }} />
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center pt-24 pb-16">
        <span ref={overlineRef} className="inline-block font-mono text-xs tracking-[0.3em] text-[var(--text-secondary)] mb-6">
          {displayOverline}
        </span>
        ...
        <p ref={subtextRef} className="text-[clamp(1rem,2vw,1.2rem)] text-[var(--text-secondary)] max-w-2xl mx-auto mb-10 leading-relaxed">
          {displaySubtext}
        </p>
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center justify-center gap-4 mb-12">
          ...
          <Button variant="outline" href="#showreel">Watch Reel</Button>
        </div>

        <div ref={statsRef} className="flex flex-wrap items-center justify-center gap-3">
          {displayStats.map((stat) => (
            <span key={stat} className="px-4 py-2 rounded-full border border-[var(--glass-border)] bg-[var(--glass-bg)] text-sm text-[var(--text-secondary)] font-mono">
              {stat}
            </span>
          ))}
        </div>
