...
import { dbGetTestimonials } from '@/lib/admin-db';
...
export default async function Testimonials() {
  let items: Testimonial[] = FALLBACK;
  try {
    const rows = await dbGetTestimonials() as Array<{ id: number; quote: string; name: string; company: string; platform: string; rating: number }>;
    if (rows.length > 0) {
      items = rows.map((r) => ({
        id: String(r.id),
        quote: r.quote,
